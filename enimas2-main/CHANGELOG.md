...
- Axis referencing now accelerates the motor onto the endstop over a ramp instead of commanding the seek rate from standstill, which made the motor slip and buzz in place until the endstop was pressed by hand
- Axis referencing gives up with `ref_failed` after a bounded travel budget instead of driving toward a switch it never reaches, and ENIMAS reports that failure immediately rather than waiting out the 30 s timeout
- A referencing confirmation split across two serial reads is no longer missed, which previously cost the full referencing timeout
- The soft travel limit for a lens shorter than the sensor-to-base-plate distance is no longer discarded by the following assignment, so such a lens can no longer be given a limit beyond the axis travel

### Safety

- Autofocus searches downwards from the retracted position instead of dropping to the lowest point first, so the stage never travels deeper than the focus plane while hunting for focus
- Autofocus clamps its descent against the physical axis travel as well as the lens-derived soft limit
- Autofocus retracts the stage when a search ends without a focus lock, instead of leaving the lens parked low over the specimen; an explicit user abort still leaves the stage where it stopped
- Motor firmware referencing behaviour changed; the firmware must be reflashed for these referencing fixes to take effect
